...
exports.createSparepart = async (req, res) => {
    try {
        const { category_id, code, name, price, stock, supplier, buy_price, discount, rack_location, brand } = req.body;
        const buy_total = (buy_price || 0) * (stock || 0);
        const [result] = await db.execute(
            'INSERT INTO spareparts (category_id, code, name, price, stock, rack_location, supplier, buy_price, buy_total, discount, brand) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [category_id || null, code || null, name, price, stock || 0, rack_location || null, supplier || null, buy_price || 0, buy_total, discount || 0, brand || null]
        );
...
exports.updateSparepart = async (req, res) => {
    try {
        ...
        const buy_total = (buy_price || 0) * (stock || 0);
        await db.execute(
            'UPDATE spareparts SET category_id=?, code=?, name=?, price=?, stock=?, rack_location=?, supplier=?, buy_price=?, buy_total=?, discount=?, brand=? WHERE id=?',
            [category_id || null, code || null, name, price, stock, rack_location || null, supplier || null, buy_price || 0, buy_total, discount || 0, brand || null, id]
        );
